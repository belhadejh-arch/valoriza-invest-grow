...
export function AdminWithdrawalsTab() {
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      toast.success(
        vars.action === "approve"
          ? t("admin.withdrawalApprovedToast")
          : t("admin.withdrawalRejectedToast"),
      );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 overflow-x-auto pb-1">
        <div className="flex items-center gap-1.5">
          {(
            [
              { id: "pending", label: t("admin.awaitingPayout") },
              { id: "approved", label: t("admin.paid") },
              { id: "rejected", label: t("admin.rejected") },
              { id: "all", label: t("admin.allWithdrawals") },
            ] as const
          ).map((tab) => {
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/50 pb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-extrabold text-foreground">
                      {content(w.username || "", { allowUserIdentifier: true })}
                    </p>
                    <p className="text-[10px] text-muted-foreground">
                      {content(w.email || "", { allowUserIdentifier: true })}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded-lg bg-surface border border-border px-2 py-0.5 text-[10px] font-mono font-bold text-cyan-glow">
                    {content(w.network || "", { allowLanguageNeutral: true })}
                  </span>
                  <div className="text-left">
                    ...
                    <span className="block text-[9px] text-muted-foreground">
                      ({t("admin.total")}: ${w.amount.toFixed(2)} | {t("admin.fee")}: ${w.fee.toFixed(2)})
                    </span>
                  </div>
                  <span
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                <div className="flex items-center justify-between rounded-xl bg-navy-deep p-2 border border-border/80">
                  <span className="text-muted-foreground">{t("admin.destinationWalletAddress")}</span>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="truncate">{content(w.address || "", { allowLanguageNeutral: true })}</span>
                  </button>
                </div>
                <div className="flex items-center justify-between rounded-xl bg-navy-deep p-2 border border-border/80">
                  <span className="text-muted-foreground">{t("admin.requestDate")}</span>
                  <span className="text-foreground">
                    {new Date(w.createdAt).toLocaleString(lang, {
                      month: "short",
                      day: "numeric",
                      hour: "2-digit",
                      minute: "2-digit",
              ...
              {w.adminNote && (
                <p className="mt-2 text-[10px] text-muted-foreground bg-surface/80 p-2 rounded-lg border border-border/50">
                  {t("admin.adminNote")}: {content(w.adminNote)}
                </p>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="w-full max-w-sm rounded-3xl border border-danger/40 bg-navy-deep p-5 shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-border/60">
              <h3 className="text-xs font-extrabold text-foreground">{t("admin.rejectWithdrawal")}</h3>
              <button
                type="button"
            ...
            <div className="mt-3 space-y-3">
              <p className="text-[11px] text-muted-foreground">
                {t("admin.withdrawalRefundReasonPrompt")}
              </p>
              <textarea
                rows={3}
                placeholder={t("admin.withdrawalRejectPlaceholder")}
                value={rejectNote}
              ...
              <button
                ...
                onClick={() =>
                  reviewMutation.mutate({
                    ...
                    action: "reject",
                    rejectReason: rejectNote || t("admin.invalidWalletAddress"),
                  })
                }
              ...
              >
                {reviewMutation.isPending ? t("admin.refunding") : t("admin.confirmRejectionAndRefund")}
              </button>
            </div>
          </div>
        </div>
