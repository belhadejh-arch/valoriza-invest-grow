...
**Why:** A connection change brought in existing accounts and funds, but not the later wheel and deposit-proof schema. Authentication could succeed while authenticated pages failed, making the problem look like broken login or a slow platform. Earlier checks against a different, empty development database were no longer representative.

**How to apply:** Compare app-used schema to the active target and confirm through an authenticated request, not just a public health check. Keep production and external-database migrations separate from development changes and obtain authorization before production writes.