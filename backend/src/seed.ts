...
export async function seedDatabase(options: SeedOptions = {}) {
  const adminEmail = (options.adminEmail ?? process.env.ADMIN_EMAIL ?? "admin@valoriza.com").trim().toLowerCase();
  const adminPassword = options.adminPassword ?? process.env.ADMIN_PASSWORD ?? "AdminValoriza2026!";
  const userEmail = (options.userEmail ?? process.env.USER_EMAIL ?? "user@valoriza.com").trim().toLowerCase();
  const userPassword = options.userPassword ?? process.env.USER_PASSWORD ?? "UserValoriza2026!";
  ...
  await withTransaction(async (client) => {
    ...
    await client.query(
      "INSERT INTO wallets (user_id, balance) VALUES ($1, 1000.0), ($2, 50.0) ON CONFLICT (user_id) DO NOTHING",
      [adminId, userId],
    );
    await client.query(
