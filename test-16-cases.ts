...
async function runTests() {
  ...
  for (const exp of expectedTiers) {
    ...
    if (!plan || plan.price !== exp.price || plan.dailyProfit !== exp.dailyProfit || plan.dailyTasks !== exp.dailyTasks) {
      test1Ok = false;
      console.error(`Mismatch for VIP ${exp.level}:`, plan);
    }
  }
  assert(test1Ok, 1, "Verify all 7 VIP tiers specifications (Prices: 13,27,61,131,273,569,1188; Profits: 0.5,1.2,2.9,6.4,13.5,28.5,60; Tasks: 2,3,4,5,6,7,8)");
  ...
  assert(
    trialRes.ok && trialState?.status === "active" && trialState.durationDays === 3 && trialState.dailyTasks === 3 && trialState.dailyProfit === 1.2,
    2,
  ...
  assert(
    firstComp.ok === true && secondComp.ok === false && secondComp.reason === "ALREADY_COMPLETED_TODAY",
    6,
  ...
  assert(
    c1.ok && c2.ok && c3.ok === false && c3.reason === "DAILY_LIMIT_EXCEEDED" && userComps.length === 2,
    7,
  ...
  const allValidYt = tasks.length === 9 && tasks.every(t => t.videoUrl.includes("youtube") && t.durationSeconds === 10);
  assert(
    allValidYt,
    9,
  ...
  const reviewRes = store.reviewDeposit(deposit.id, "approve", "admin-tester", "Approved by QA test");
  const postApprovalBalance = store.getWallet(depUser).balance;
  assert(
    reviewRes.ok && reviewRes.deposit?.status === "approved" && postApprovalBalance === preApprovalBalance + 100,
    16,
